 ...
 * - используйте текущий год либо непосредственно в своем коде, либо с помощью глобальной переменной, например, carYear
...
var carYear = new Date().getFullYear(); // получить текущий год как число
...
var car = {
  ...
  info: carinfo,
  get used() {
    return this.year !== carYear ? 'used' : 'new';
  },
  set used(value) {
    if (value === 'new' && this.year < carYear) this.year = carYear;
  }
};
var car2 = {
  engine: 5000,
  ...
  info: carinfo,
  get used() {
    return carYear - this.year ? 'used' : 'new';
  },
  set used(value) {
    if (value === 'new' && this.year < carYear) this.year = carYear;
  }
...
var myMax = function myMax(arr) { return (Math.max.apply(null, arr));}
