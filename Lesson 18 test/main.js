...
var wordsList = (str, subStr) => {
    ...
    var arr = str
       ...
       .toLowerCase()
       .split(' ')
       .filter((setItem) => setItem.indexOf(subStr) > -1);
  var res = new Set();
...
var getDateAgo = (date, days) => {
  ...
  return dateCopy.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1');
}
 ...
 *
 * Например:
 * let car = new Car(5000, 'Mustang', 'Ford', 2021);
...
var car = new Car(5000, 'Mustang', 'Ford', 2021);
